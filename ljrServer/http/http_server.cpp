...
namespace ljrserver
{
    ...
    namespace http
    {
        ...
        HttpServer::HttpServer(bool keepalive, ljrserver::IOManager *worker, ljrserver::IOManager *accept_worker)
        ...
        void HttpServer::handleClient(Socket::ptr client)
        {
            ...
            do
            {
                ...
                if (!req)
                {
                    LJRSERVER_LOG_WARN(g_logger) << "recv http request fail, errno = " << errno
                                                 << " errno-string = " << strerror(errno)
                                                 << " client:" << *client;
                    break;
                }

                HttpResponse::ptr rsp(new HttpResponse(req->getVersion(), req->isCose() || !m_isKeepalive));
