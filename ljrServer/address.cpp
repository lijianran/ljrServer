...
namespace ljrserver
{
    ...
    ***************/
                         ...
                         int family, int type, int protocol)
    {
        ...
        if (error)
        {
            LJRSERVER_LOG_ERROR(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result, int family)
    {
        ...
        {
            LJRSERVER_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs error = "
                                          << errno << " error-string = " << strerror(errno);
            return false;
        }
        ...
        {
            LJRSERVER_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception: " << e.what();
            freeifaddrs(results);
            return false;
        }
    ...
    ***************/
    ...
    IPAddress::ptr IPAddress::Create(const char *address, uint16_t port)
    {
        ...
        if (error)
        {
            LJRSERVER_LOG_ERROR(g_logger) << "IPAddress::Create(" << address
                                          << ", " << port << ") error = " << error
                                          << " errno = " << errno
    ...
    ***************/
    ...
    IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            LJRSERVER_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", "
    ...
    ***************/
    ...
    IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port)
    {
        ...
        if (result <= 0)
        {
            LJRSERVER_LOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", "
                                          << port << ") rt = " << result << " errno = " << errno
                                          << " errno-string = " << strerror(errno);
            return nullptr;
        }
    ...
    ***************/
    ...
    UnkonwnAddress::UnkonwnAddress(const sockaddr &addr)
    {
        m_addr = addr;
    }

    const sockaddr *UnkonwnAddress::getAddr() const
    {
        return (sockaddr *)&m_addr;
    }

    sockaddr *UnkonwnAddress::getAddr()
    {
        return (sockaddr *)&m_addr;
    }

    socklen_t UnkonwnAddress::getAddrLen() const
    {
        return sizeof(m_addr);
    }
