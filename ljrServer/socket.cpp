...
namespace ljrserver
{
    ...
    bool Socket::getOption(int level, int option, void *result, size_t *len)
    {
        ...
        if (rt)
        {
            LJRSERVER_LOG_DEBUG(g_logger) << "getOption sock = " << m_sock
                                          << " level = " << level
                                          << " option = " << option
                                          << " errno = " << errno
    ...
    bool Socket::setOption(int level, int option, const void *result, size_t len)
    {
        ...
        {
            LJRSERVER_LOG_DEBUG(g_logger) << "setOption sock = " << m_sock
                                          << " level = " << level
                                          << " option = " << option
                                          << " errno = " << errno
                                          << " errno-string = " << strerror(errno);
    ...
    Socket::ptr Socket::accept()
    {
        ...
        if (newsock == -1)
        {
            LJRSERVER_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno = " << errno << " errno-string = " << strerror(errno);
            return nullptr;
        }
    ...
    bool Socket::bind(const Address::ptr addr)
    {
        ...
        {
            LJRSERVER_LOG_ERROR(g_logger) << "bind sock.family(" << m_family
    ...
    bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms)
    {
        ...
        if (LJRSERVER_UNLICKLY(addr->getFamily() != m_family))
        {
            LJRSERVER_LOG_ERROR(g_logger) << "connect sock.family(" << m_family
                                          << ") addr.family(" << addr->getFamily()
                                          << ") not equal, addr = " << addr->toString();
            return false;
        }
        ...
        if (timeout_ms == (uint64_t)-1)
        {
            ...
            {
                LJRSERVER_LOG_ERROR(g_logger) << "sock = " << m_sock
    ...
    Address::ptr Socket::getRemoteAddress()
    {
        ...
        {
            LJRSERVER_LOG_ERROR(g_logger) << "getpeername error sock = " << m_sock
                                          << " errno = " << errno
                                          << " errno-string = " << strerror(errno);
    ...
    Address::ptr Socket::getLocalAddress()
    {
        ...
        {
            LJRSERVER_LOG_ERROR(g_logger) << "getsockname error sock = " << m_sock
                                          << " errno = " << errno
                                          << " errno-string = " << strerror(errno);
            return Address::ptr(new UnkonwnAddress(m_family));
        }
    ...
    {
        return IOManager::GetThis()->cancelEvent(m_sock, ljrserver::IOManager::READ);
    }

    bool Socket::cancelWrite()
    {
        return IOManager::GetThis()->cancelEvent(m_sock, ljrserver::IOManager::WRITE);
    }
    ...
    void Socket::newSock()
    {
        ...
        {
            LJRSERVER_LOG_ERROR(g_logger) << "socket(" << m_family << ", " << m_type << ", "
